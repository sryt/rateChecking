package com.example.grocerieslist.db.company;

import android.content.Context;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;

import com.example.grocerieslist.utilities.Constant;

/**
 * Created by Tejaswi on 30/06/21.
 */
public class CompanyDB extends SQLiteOpenHelper {
    private static final int DATABASE_VERSION = 1;
    private static final String DATABASE_NAME = Constant.DB_Company;
    public static final String TABLE = "companies";
    public static final String KEY_ID = "id";
    public static final String KEY_COMPANY_NAME = "name";
    public static final String KEY_COMPANY_DESC = "description";
    public static final String KEY_COMPANY_OWNER = "owner";
    public static final String KEY_ADDRESS = "address";
    public static final String KEY_PLACE = "place";
    public static final String KEY_PINCODE = "pincode";
    public static final String KEY_PHONENUMBER = "phonenumber";
    public static final String KEY_EMAILID = "emailid";
    public static final String KEY_GSTNO = "gstno";
    public static final String KEY_ACCNUMBER = "accnumber";
    public static final String KEY_BANKNAME = "bankname";
    public static final String KEY_IFSCCODE = "ifscode";
    public static final String KEY_BANKBRANCH = "bankbranch";
    public static final String KEY_FLAG = "flag";
    public static final String KEY_STATUS = "status";

    public CompanyDB(Context context) {
        super(context, DATABASE_NAME, null, DATABASE_VERSION);
        //3rd argument to be passed is CursorFactory instance
    }
    // Creating Tables
    @Override
    public void onCreate(SQLiteDatabase db) {
        String CREATE_TABLE = "CREATE TABLE " + TABLE + "("
                + KEY_ID + " INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL ,"
                + KEY_COMPANY_NAME + " TEXT,"
                + KEY_COMPANY_DESC + " TEXT,"
                + KEY_COMPANY_OWNER + " TEXT,"
                + KEY_ADDRESS + " TEXT,"
                + KEY_PLACE + " TEXT,"
                + KEY_PINCODE + " TEXT,"
                + KEY_GSTNO + " TEXT,"
                + KEY_PHONENUMBER + " TEXT,"
                + KEY_EMAILID + " TEXT,"
                + KEY_ACCNUMBER + " TEXT,"
                + KEY_BANKNAME + " TEXT,"
                + KEY_BANKBRANCH + " TEXT,"
                + KEY_IFSCCODE + " TEXT,"
                + KEY_FLAG + " TEXT,"
                + KEY_STATUS + " TEXT" + ")";
        db.execSQL(CREATE_TABLE);
    }

    // Upgrading database
    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        // Drop older table if existed
        db.execSQL("DROP TABLE IF EXISTS " + TABLE);

        // Create tables again
        onCreate(db);
    }
}
